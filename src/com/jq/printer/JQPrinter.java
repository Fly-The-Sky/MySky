...
public class JQPrinter {
	...
	// 打开端口
	// 注意：最好不要将此函数放在Activity的onCreate函数中，因为bluetooth
	// connect时会有定的延时，有时会造成页面显示很慢，而误认为没有点击按钮
	public boolean open(String btDeviceString) {
		if (isOpen)
			return true;
		if (btDeviceString == null)
			return false;
		if (!_port.open(btDeviceString, 3000))
			return false;
		isOpen = true;
		return true;
	}
	...
	/*
	 * 唤醒打印机 注意:部分手持蓝牙连接第一次不稳定，会造成开头字符乱码，可以通常这个方法来避免此问题
	 */
